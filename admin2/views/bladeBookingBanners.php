...
if( isset($_GET["hide"]) && !empty($_GET["hide"]) ){
	if( updateDB('tbl_banners',array('hidden'=> '2'),"`id` = '{$_GET["hide"]}'") ){
		header("LOCATION: ?v=BookingBanners");
	}
}

if( isset($_GET["show"]) && !empty($_GET["show"]) ){
	if( updateDB('tbl_banners',array('hidden'=> '1'),"`id` = '{$_GET["show"]}'") ){
		header("LOCATION: ?v=BookingBanners");
	}
}

if( isset($_GET["delId"]) && !empty($_GET["delId"]) ){
	if( updateDB('tbl_banners',array('status'=> '1'),"`id` = '{$_GET["delId"]}'") ){
		header("LOCATION: ?v=BookingBanners");
	}
}
...
if( isset($_POST["updateRank"]) ){
	for( $i = 0; $i < sizeof($_POST["rank"]); $i++){
		updateDB('tbl_banners',array("rank"=>$_POST["rank"][$i]),"`id` = '{$_POST["id"][$i]}'");
	}
	header("LOCATION: ?v=BookingBanners");
}
...
if( isset($_POST["title"]) ){
	...
	if ( $id == 0 ){
		...
		if (is_uploaded_file($_FILES['image']['tmp_name'])) {
            ...
            $imageurl = selectDB('tbl_banners', "`id` = '{$id}'");
            $_POST["image"] = $imageurl[0]["image"];
        }
		if( updateDB('tbl_banners', $_POST, "`id` = '{$id}'") ){
			header("LOCATION: ?v=BookingBanners");
		}else{
		?>
		<script>
...
<div class="table-responsive">
	<table class="table display responsive product-overview mb-30" id="myTable">
		...
		<tbody>
		<?php 
		if( $banners = selectDB('tbl_banners',"`status` = '0' ORDER BY `rank` ASC") ){
		...
		<div style="display:none"><label id="type<?php echo $banners[$i]["id"]?>"><?php echo $banners[$i]["type"] ?></label></div>
		<div style="display:none"><label id="link<?php echo $banners[$i]["id"]?>"><?php echo $banners[$i]["link"] ?></label></div>
