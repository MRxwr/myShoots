    	...
    	<!-- Title -->
				<div class="row heading-bg">
					<div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
						<h5 class="txt-dark">Booking Duplicates </h5>
					</div>
	...
	?>
            ...
            //                                     ";
                                                ...
                                                // First, find the duplicate booking_date and booking_time
                                                $query_duplicates = "
                                                    ...
                                                    HAVING COUNT(*) > 1
                                                ";
                                                $res_duplicates = $obj->execute_query($conn, $query_duplicates);
                                                
                                                // Step 2: Fetch all rows that have the same booking_date and booking_time
                                                if (mysqli_num_rows($res_duplicates) > 0) {
                                                    $duplicate_slots = [];
                                                
                                                    // Collect the duplicate booking_date and booking_time
                                                    while ($row = mysqli_fetch_assoc($res_duplicates)) {
                                                        $duplicate_slots[] = "('".$row['booking_date']."', '".$row['booking_time']."')";
                                                    }
                                                
                                                    // Join the duplicates as a condition in the next query
                                                    $duplicate_conditions = implode(", ", $duplicate_slots);
                                                
                                                    // Now fetch all the duplicate rows
                                                    $query_all_duplicates = "
                                                        SELECT *
                                                        FROM $tbl_name
                                                        WHERE status='Yes'
                                                        AND (booking_date, booking_time) IN ($duplicate_conditions)
                                                        ORDER BY booking_date DESC, booking_time DESC
                                                    ";
                                                    $res = $obj->execute_query($conn, $query_all_duplicates);
												$sn = 1;
												if($res)
												{
														...
														while ($row=$obj->fetch_data($res)) {
															...
															<tr>
															   ...
															   <td>
																	...
																	<?php }else{?>
																	<a  class="btn btn-primary d-inline  btn-xs bookingrefund" onclick="bookingRefund(<?=$id?>)"  id="<?=$id?>" style="margin-bottom: 10px;" ><?=$lang['refund']?></a>
																	<?php } ?>
