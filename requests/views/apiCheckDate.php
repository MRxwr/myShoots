    ...
    if( ($selectedDate >= $openDate) && ($selectedDate <= $closeDate) ){
        ...
        if( $bookingTable = selectDB("tbl_bookings", "`booking_date`='{$selectedDate}' AND `status`='Yes'") ){
            ...
            if(count($freeTimes) > 0){
                echo outputData(array("message"=>"Available times for this date. {$freeTimes}", "available_times"=>array_values($freeTimes)));
            }else{
                echo outputError(array("message"=>"No available times for this date."));
            }
        }else{
            echo outputData(array("message"=>"Available times for this date. {$availableTimes}", "available_times"=>array_values($availableTimes)));
        }
    }else{
